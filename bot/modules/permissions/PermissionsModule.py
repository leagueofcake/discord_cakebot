...
class PermissionsModule(ModuleInterface):
    ...
    def auth_function(self, f):
        async def ret_fun(message, owner_auth=False, manage_guild_auth=False, require_non_cakebot=False,
                          cakebot_perm=None):
            owner_check = owner_auth and PermissionsModule._is_owner(message.author)
            manage_guild_check = manage_guild_auth and PermissionsModule._can_manage_guild(message.author, message.channel)
            ...
            if is_cakebot_check and (no_auth or owner_check or manage_guild_check or cakebot_perm_check):
                await f(message)
            else:
    ...
    async def _set_permissions(self, message, user):
        ...
        await self.auth_function(inner)(message, require_non_cakebot=True, manage_guild_auth=True, owner_auth=True)
